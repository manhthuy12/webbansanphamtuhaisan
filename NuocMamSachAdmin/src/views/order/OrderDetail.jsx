    ...
    <div className="bg-white p-6 rounded-lg shadow-lg w-full flex space-x-6">
      ...
      <div className="w-2/3 space-y-6">
        ...
        <div className="border border-gray-300 rounded-lg p-4">
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex space-x-4">
              <span className="bg-yellow-100 text-yellow-600 py-1 px-3 rounded-full">{order.status}</span>
              <span className="bg-red-100 text-red-600 py-1 px-3 rounded-full">{order.paid ? 'Đã thanh toán' : 'Chưa thanh toán'}</span>
            </div>
          </div>
            ...
            <div className="mb-6" key={item.id}>
              <div className="flex justify-between items-center mb-2">
                ...
                <div>
                  ...
                  <p className="font-semibold text-lg">{formatCurrency(item.priceAtPurchase * item.quantity)}</p>
                </div>
              </div>

              {item.accessoryItems && item.accessoryItems.length > 0 && (
                <div className="ml-20">
                  <h4 className="text-gray-500 font-semibold mb-2">Phụ kiện:</h4>
                  {item.accessoryItems.map((accessory, index) => (
                    <div key={index} className="flex justify-between items-center mb-2">
                      <div className="flex items-center space-x-2">
                        {accessory.accessory.image && (
                          <img src={accessory.accessory.image} alt={accessory.accessory.name} className="w-8 h-8 object-cover rounded" />
                        )}
                        <p className="text-sm text-gray-500">
                          {accessory.accessory.name} (x {accessory.quantity})
                        </p>
                      </div>
                      <p className="text-sm text-gray-500">{formatCurrency(accessory.priceAtPurchase * accessory.quantity)}</p>
                    </div>
                  ))}
                </div>
              )}
        ...
        <div className="border border-gray-300 rounded-lg p-4">
          ...
          <div className="flex justify-between mb-2">
            ...
            <p>- {formatCurrency(order.voucher?.discountValue || 0)}</p>
          </div>
      ...
      <div className="w-1/3 border border-gray-300 rounded-lg p-4">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-500">{order.addressBook.phoneNumber || 'Không có số điện thoại'}</p>
        </div>
        <div className="mb-6">
        ...
        <div>
          ...
          <p>
            {order?.addressBook.address}, {order?.addressBook.ward}, {order?.addressBook.district}, {order?.addressBook.city}
          </p>
