...
const CreateProductModal = ({ open, onClose, fetchProducts, categories }) => {
  const [form] = Form.useForm();
  const [loading, setLoading] = useState(false);
  const [variants, setVariants] = useState([]); // Danh sách biến thể sản phẩm
  const [fileList, setFileList] = useState([]); // Hình ảnh sản phẩm
  const [description, setDescription] = useState(''); // Mô tả sản phẩm
  const [variantForm] = Form.useForm(); // Form thêm biến thể
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      const productData = {
        ...
        variants: sanitizedVariants, // Sử dụng variants đã loại bỏ id
        description,
      };
  ...
  return (
    <Modal title="Tạo mới sản phẩm" visible={open} onCancel={onClose} footer={null} width={800}>
